...
public class KafkaProducer {
     ...
     * @param topic   ：主题名称，不存在时自动创建，默认1个分区，无副本。主题名称也可以通过配置文件配置，这里直接通过参数传入。
     */
    public String sendMessage(String topic, Object body) {
        logger.info("向指定主题发送信息，topic={},message={}", topic, body);
        SendResult<String, Object> sendResult = null;
        try {
            // 异步
            // kafkaTemplate.send(topic, valueAsString);
            // 同步：get() 获取执行结果，此时线程将阻塞，等待执行结果
            sendResult = kafkaTemplate.send(topic, JSON.toJSONString(body)).get();
